...
public class GroupConversationInfoFragment extends Fragment implements ConversationMemberAdapter.OnMemberClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private void showGroupManageViews() {
        ...
        if ("1".equals(userViewModel.getUserSetting(UserSettingScope.GroupHideNickname, groupInfo.target))){
            showGroupMemberNickNameSwitchButton.setChecked(false);
        }else {
            showGroupMemberNickNameSwitchButton.setChecked(true);
        }
