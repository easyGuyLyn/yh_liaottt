...
public class GroupViewModel extends ViewModel implements OnGroupInfoUpdateListener, OnGroupMembersUpdateListener {
    ...
    String generateGroupPortrait(Context context, List<UserInfo> userInfos) throws Exception {
        ...
        for (UserInfo userInfo : userInfos) {
            Drawable drawable;
            try {
                drawable = GlideApp.with(context).load(userInfo.portrait).error(R.mipmap.im_avatar_def).submit(60, 60).get();
            } catch (Exception e) {
                e.printStackTrace();
                drawable = GlideApp.with(context).load(R.mipmap.im_avatar_def).submit(60, 60).get();
            }
