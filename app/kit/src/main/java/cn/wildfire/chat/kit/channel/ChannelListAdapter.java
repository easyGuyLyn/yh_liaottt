...
public class ChannelListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == R.layout.im_channel_item_category) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.im_channel_item_category, parent, false);
            return new CategoryViewHolder(view);
        } else {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.im_channel_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == R.layout.im_channel_item_category) {
            if (position == 0) {
    ...
    public int getItemViewType(int position) {
        if (createdChannels == null || createdChannels.isEmpty()) {
            ...
            switch (position) {
                case 0:
                case 1:
                    type = R.layout.im_channel_item_category;
                    break;
                default:
                    type = R.layout.im_channel_item;
                    break;
            }
            return type;
        } else {
            if (position == 0 || position == createdChannels.size() + 1) {
                return R.layout.im_channel_item_category;
            } else {
                return R.layout.im_channel_item;
            }
        }
    }
