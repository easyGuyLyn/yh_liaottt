    ...
    private void onReceiveMessage(final List<Message> messages, final boolean hasMore) {
        mainHandler.post(() -> {
            ...
            for (Message message : messages) {
                if (message.content instanceof RedPacketCloseMessageContent) {   // 牛牛红包结束通知
                    Log.e("RedPacketCloseMessa", "收到消息");
                    try {
                        RedPacketCloseMessageContent redPacketCloseMessageContent = (RedPacketCloseMessageContent) message.content;
                        Long aLong = Long.valueOf(redPacketCloseMessageContent.redPacketMessageId);
                        Message messageByUid = ChatManager.Instance().getMessageByUid(aLong);
                        MessageContent messageContent = messageByUid.content;
                        if (messageContent instanceof NiuniuRedPacketMessageContent) {
                            NiuniuRedPacketMessageContent niuniuRedPacketMessageContent = (NiuniuRedPacketMessageContent) messageContent;
                            niuniuRedPacketMessageContent.statues = "3";
                            ChatManager.Instance().updateMessage(messageByUid.messageId, niuniuRedPacketMessageContent);
                        } else if (messageContent instanceof SweepRedPacketMessageContent) {
                            SweepRedPacketMessageContent sweepRedPacketMessageContent = (SweepRedPacketMessageContent) messageContent;
                            sweepRedPacketMessageContent.statues = "3";
                            ChatManager.Instance().updateMessage(messageByUid.messageId, sweepRedPacketMessageContent);
                        } else {
                            RedPacketMessageContent redPacketMessageContent = (RedPacketMessageContent) messageContent;
                            redPacketMessageContent.statues = "3";
                            ChatManager.Instance().updateMessage(messageByUid.messageId, redPacketMessageContent);
                        }
                    } catch (Exception e) {
                        Log.e("RedPacketCloseMessa", "更新消息异常" + e.toString());
                    }
                ...
                if ((message.content instanceof QuitGroupNotificationContent && ((QuitGroupNotificationContent) message.content).operator.equals(getUserId()))
                        ...
                        || message.content instanceof DismissGroupNotificationContent || message.content instanceof RedPacketCloseMessageContent) {
