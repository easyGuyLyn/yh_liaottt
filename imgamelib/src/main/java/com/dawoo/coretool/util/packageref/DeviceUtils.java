...
public final class DeviceUtils {
    ...
    public static String getLocaleLanguage(Context context)
    {
        ...
        if(Build.VERSION.SDK_INT >=Build.VERSION_CODES.N )
        {
            ...
            if(null != localeList && !localeList.isEmpty())
            {
                locale = localeList.get(0).getLanguage();
            }
        }
        else
        {
            if(null != context.getResources().getConfiguration().locale)
            {
    ...
    private static String getLocalMacAddressFromIp(){
        ...
        try {
            LogUtils.e("本地的Ip地址 "+inetAddress.toString());
            byte[] bb  = NetworkInterface.getByInetAddress(inetAddress).getHardwareAddress();
            if(null == bb){
                LogUtils.e("本地ip转MAC地址失败 "+strMacAdd);
                String deviceId = getAndroidID();
                if(TextUtils.isEmpty(deviceId))
                {
                    deviceId =Build.BRAND+Build.SERIAL+Build.DEVICE;
            ...
            for(int i=0;i<bb.length;i++){
                ...
                String string = Integer.toHexString(bb[i] & 0xFF);
                stringBuffer.append(string.length() == 1 ? 0+string : string);
            }
            strMacAdd = stringBuffer.toString().toUpperCase();
            LogUtils.e("本地的MAC地址 "+strMacAdd);
        } catch (Exception e) {
            e.printStackTrace();
                LogUtils.e("Exception 本地ip转MAC地址失败 "+strMacAdd);
                String deviceId = getAndroidID();
                if(TextUtils.isEmpty(deviceId))
                {
                    deviceId =Build.BRAND+Build.SERIAL+Build.DEVICE;
