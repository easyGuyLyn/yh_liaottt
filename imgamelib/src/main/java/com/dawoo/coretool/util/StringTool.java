...
//        return map;
     ...
     * @return
     */
    public static String getCoverRealName(String realName) {
        if (!StringTool.isEmpty(realName)) {
            int length = realName.length();
            if (1 == length) {
                return "*";
            } else if (2 == length) {
                char ch = realName.charAt(realName.length()-1);
                return realName.replace(ch, '*');
            } else {
                char firstStr = realName.charAt(0);
                char lastStr = realName.charAt(realName.length()-1);
                String subStr = realName.substring(1, realName.length() - 1);
